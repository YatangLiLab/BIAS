...
class evalDADA2KS(Dataset):
    def __init__(self, root_path, phase, interval=1, transforms={'image':None, 'salmap': None, 'fixpt': None}, 
                       use_salmap=True, use_fixation=False, data_aug=False):
        self.root_path = root_path
    ...
    def get_data_list(self):
        fileIDs = os.listdir(self.root_path)
        fileIDs = [os.path.join(self.root_path, f) for f in fileIDs]
        for id_ in fileIDs:
    ...
    def read_video(self, video_file, toGray=False):
        """Read video frames
        """
        assert os.path.exists(video_file), "Path does not exist: %s"%(video_file)
        # get the video data
        ...
        for fid in range(n_frame):
            ...
            video_data.append(frame)
        video_data = np.array(video_data, dtype=np.float32)  # 4D tensor (1, H, W, 3)
        return video_data
    ...
    def gather_info(self, index):
        ...
        """
        # video file info
        accident_id = 0# int(self.data_list[index].split('/')[0])
        video_id = 0 # int(self.data_list[index].split('/')[1])
        # toa info
        start, end = 0,0# self.clips[index]
        # if self.labels[index] > 0:  # positive sample
        #     assert self.toas[index] >= start and self.toas[index] <= end, "sample id: %s"%(self.data_list[index])
        #     toa = int((self.toas[index] - start) / self.interval)
        # else:
        #     toa = int(self.toas[index])  # negative sample (toa=-1)
        toa = -1
        data_info = np.array([accident_id, video_id, start, end, 0, toa], dtype=np.int32)
        return data_info
