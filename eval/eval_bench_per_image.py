...
def calculate_metrics(video_name, temp_predictions_path, temp_gt_saliency_path, temp_gt_fixations_path,num_workers):
    ...
    return [{'video_name' : video_name,
             'index'  : index,
            'cc' : conv_scores['cc_score'][index],
            'sim' : conv_scores['sim_score'][index],
            'nss' : conv_scores['nss_score'][index],
            'auc_judd' : conv_scores['auc_judd_score'][index],
            'kldiv' : conv_scores['kldiv_score'][index],
            'auc_shuffled_score': conv_scores['auc_shuffled_score'][index]} for index in range(length)]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--models_root', help='path to directory with models predictions', default='')
    ...
    potential_models = ['single_image']
    models = [model for model in models if model in potential_models]
    ...
    for model_num, model_root in enumerate(models):
        ...
        with open(f'{json_root}_perimage.json', mode='w') as output:
        ...
        header = not path.exists(f'{result_name}_perimage.csv')
