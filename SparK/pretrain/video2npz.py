...
# print(name_list)
sm_vid_root = '/mnt/e/Li Lab/MS-TCA/MyMSTCN/sm_vids'
videos = [os.path.join(sm_vid_root, name+'.mp4') for name in name_list]
for vid in tqdm.tqdm(videos):
    assert os.path.exists(vid)
print('all videos exist, start processing.')

from models.resnet18_inference import SaliencyMap_INF
from timm.models import create_model

dense_encoder = create_model('SaliencyMap_INF')# SaliencyMap()
dense_encoder.load_state_dict(torch.load(os.path.join('..','ResNet18_MGF_197epoch','SaliencyMap_1kpretrained_timm_style.pth')))
# dense_encoder.to_dense()
# convert_sparse_bn_to_regular_bn(dense_encoder)
dense_encoder.eval()
dense_encoder = dense_encoder.cuda()

tgt_npz_folder = '/mnt/e/Li Lab/MS-TCA/MyMSTCN/data/Saldataset/SMfeature_npy'
tgt_path = pathlib.Path(tgt_npz_folder)
tgt_path.mkdir(parents=True, exist_ok=True)

for idx, vid_name in enumerate(tqdm.tqdm(videos)):
