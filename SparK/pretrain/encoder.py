...
def _get_active_ex_or_ii(H, W, returning_active_ex=True):
    """
    返回：
      returning_active_ex=True  -> (B,1,H,W) float mask
      returning_active_ex=False -> (bi, hi, wi) long index tuple
    """
...
class SparseBasicBlock(BasicBlock):
    ...
    def forward(self, x):
        ...
        out = self.bn1(out)
        out = self.act1(out)
        out *= _get_active_ex_or_ii(H=out.shape[2], W=out.shape[3], returning_active_ex=True)
        ...
        if self.downsample is not None:
            ...
            identity *= _get_active_ex_or_ii(H=identity.shape[2], W=identity.shape[3], returning_active_ex=True)
