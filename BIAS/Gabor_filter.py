...
if __name__ == "__main__":
    ...
    image = cv2.imread("test_image\\4_dirs.jpg")
    image = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY).astype(np.float32)
    image = cv2.resize(image, (image.shape[1]//1, image.shape[0]//1))
    image /= 255
    image1,image2,image3,image4 = np.copy(image),np.copy(image),np.copy(image),np.copy(image)
    Orientation = np.float32(np.pi * 3/ 4)
    Frequency = np.float32(2* np.pi**2/2.7)
    kernal_size = 4
    gabor_filter(gabor_lib,image1,0, Frequency, kernal_size)
    gabor_filter(gabor_lib,image2,np.pi/4, Frequency, kernal_size)
    gabor_filter(gabor_lib,image3,np.pi/2, Frequency, kernal_size)
    gabor_filter(gabor_lib,image4,np.pi * 3 / 4, Frequency, kernal_size)
