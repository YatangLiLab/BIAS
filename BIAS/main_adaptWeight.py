...
def parse_args():
    ...
    # 
    parse.add_argument('--video_path', default="E:\\your_path\\video\\video\\video\\230.avi", type=str, help='path of sample video') 
...
def main(args:argparse.Namespace):
    ...
    for _count in tqdm.trange(int(frame_number)): # in range(int(frame_number)): #
        ...
        weight_static = max(norm_static.max(),1e-6)
        weight_dynamic = max(norm_dynamic.max(),1e-6)
        # weight_multi = max(norm_multi_rst.max(),1e-6)
        S_bar = addition(weight_static * norm_static , weight_dynamic * norm_dynamic)/(weight_static + weight_dynamic)
        # print("static: {}, dynamic: {}, multi: {}".format(weight_static,weight_dynamic,weight_multi))
