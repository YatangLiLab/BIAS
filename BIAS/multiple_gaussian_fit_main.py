...
def parse_args():
    ...
    # 
    parse.add_argument('--video_path', default=r"E:\YourPath(some logs are changed after generated only for anonymity)\ChunpengVideos\videos\bugs", type=str, help='path of sample video') 
    # parse.add_argument('--video_path', default="E:\\your_path\\CarCrashVideo\\sample_video.mp4", type=str, help='path of sample video') 
    parse.add_argument('--generate_name',default = "result_video\\MGF663.mp4",type=str,help = 'default generate name')
...
def main(args:argparse.Namespace):
    ...
    static_saliency_map = loop_for_multiple_gaussian_fitting2(static_saliency_map)
    ...
    for _count in tqdm.trange(int(frame_number)): # in range(int(frame_number)): #
        ...
        S_bar = loop_for_multiple_gaussian_fitting2(S_bar)
        if args.continuity:
             if _count == 0:
