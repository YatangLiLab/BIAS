...
def main(args:argparse.Namespace):
    ...
    for _count in tqdm.trange(1,int(frame_number)): # in range(int(frame_number)): #
        ...
        norm_dynamic = normalize_img(diff_time_scale_map,norm_lib)
    ...
    with h5py.File("h5rst\\adjusted_motion_saliency_map.h5", "a") as hf:
        dataset_name = f"{video_index}_motion_saliency_map"
        hf.create_dataset(dataset_name, data=image_saliency_map, compression='gzip', compression_opts=9)
