...
from mgf_utils import loop_for_multiple_gaussian_fitting2
...
def parse_args():
    ...
    parse.add_argument('--generate_name',default = "result_video\\Levy.mp4",type=str,help = 'default generate name')
    ...
    parse.add_argument('--LevyK',type = int, default= 500, help = "static, dynamic or both.")
...
def main(args:argparse.Namespace):
    ...
    levy = FastLevyFlight(beta=1.0, D=0.8, T=1.5, R_max=None, seed=42)
    if not ret:
    ...
    levy.set_saliency_shape(*static_saliency_map.shape)
    traj = levy.walk(args.LevyK, static_saliency_map)
    static_saliency_map = levy.line_density(traj,static_saliency_map.shape[0],static_saliency_map.shape[1])
    ...
    for _count in tqdm.trange(int(frame_number)): # in range(int(frame_number)): #
        ...
        if args.continuity:
            if _count == 0:
                ...
                tmp_dynamic = norm_dynamic / (1-tmp_save_factor)
            else:
        ...
        # levy.set_saliency_shape(*S_bar.shape)
        traj = levy.walk(args.LevyK, S_bar)
        S_bar = levy.line_density(traj, S_bar.shape[0], S_bar.shape[1])
        if args.continuity:
             if _count == 0:
                 tmp_S_bar = S_bar / (1-tmp_save_factor)
             else:
                 tmp_S_bar = tmp_S_bar * tmp_save_factor + S_bar
                 S_bar = tmp_S_bar * (1-tmp_save_factor)
        ...
        long_edge_gaussian_kernal = cv2.getGaussianKernel(S_bar.shape[0],S_bar.shape[0])
        long_edge_gaussian_kernal /= np.max(long_edge_gaussian_kernal)
        short_edge_gaussian_kernal = cv2.getGaussianKernel(S_bar.shape[1],S_bar.shape[1])
        short_edge_gaussian_kernal /= np.max(short_edge_gaussian_kernal)
        S_bar = long_edge_gaussian_kernal * S_bar * short_edge_gaussian_kernal.T
        S_bar = cv2.GaussianBlur(S_bar,(15,15),sigmaX=0.1*S_bar.shape[0],sigmaY=0.1*S_bar.shape[1])
        # S_bar = np.where(S_bar > np.median(S_bar),S_bar,0)
