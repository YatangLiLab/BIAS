#include <cmath>
#include <stdio.h>
int constant_index(int var, int change, int bound){
...
int mirror_index(int var, int change, int bound) {
    ...
    if (current_result < 0) {
        return mirror_index(-current_result - 1,0,bound); 
    } else if (current_result >= bound) {
        return mirror_index(2* bound - 1 - current_result,0,bound); 
    } else {
        return current_result; 
    }
}
        ...
        for (int y = 0; y < M; ++y){
            for (int _x = 0; _x < N; ++_x){
                ...
                for (int _kernel = -ksize; _kernel<ksize+1;++_kernel){
                    RJ[y][_x] += cos_wc_x[_x] * \
                    cos_c[_kernel + ksize] * image[y * N + constant_index(_x,_kernel,N)] * S_sigma[_kernel + ksize] \
                    + sin_wc_x[_x] * \
                    sin_c[_kernel + ksize] * image[y * N + constant_index(_x,_kernel,N)] * S_sigma[_kernel + ksize];
                    IJ[y][_x] += - cos_wc_x[_x] * \
                    sin_c[_kernel + ksize] * image[y * N + constant_index(_x,_kernel,N)] * S_sigma[_kernel + ksize] \
                    + sin_wc_x[_x] * \
                    cos_c[_kernel + ksize] * image[y * N + constant_index(_x,_kernel,N)] * S_sigma[_kernel + ksize];
            ...
            for (int _y = 0; _y < M; ++_y){
                ...
                for (int __kernel = - ksize; __kernel < ksize+1; ++__kernel){
                    F[_y][x] += cos_ws_y[_y] * S_sigma[__kernel + ksize] *\
                            (cos_s[__kernel + ksize] * RJ[constant_index(_y,__kernel,M)][x]) \
                           + sin_s[__kernel + ksize] * IJ[constant_index(_y,__kernel,M)][x]  \
                              + sin_ws_y[_y] * S_sigma[__kernel + ksize] *\
                            (sin_s[__kernel + ksize] * RJ[constant_index(_y,__kernel,M)][x]\
                           - cos_s[__kernel + ksize] * IJ[constant_index(_y,__kernel,M)][x])  ;
...
// g++ -O3 -ffast-math  -shared -o gabor_filter.dll gabor_filter.cpp
// 0.0019989013671875
