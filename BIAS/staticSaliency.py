...
def non_linear(x):
    t = (x - np.min(x)) / (np.max(x) - np.min(x) + 1e-7)
...
def parse_args():
    ...
    parse.add_argument('--video_path', default="E:\\Li Lab\\itti_and_lif\\RealtimeSaliency\\test_video\\001.avi", type=str, help='path of sample video') 
    parse.add_argument('--generate_name',default = "result_video\\1919810.mp4",type=str,help = 'default generate name')
    ...
    parse.add_argument('--alpha_beta',default = (0.3,0.3),type=tuple,help = 'determine standard alpha and beta.')
    parse.add_argument('--output',default="test",type = str, help = "determine output format.")
    parse.add_argument('--target_folder',default='lower_result\\tests',type=str,help = 'default place to place pred results')
    parse.add_argument('--center',default = (1,2,3),type=tuple,help = "center params. Itti default params are (2, 3, 4), equals to (1,2,3) here, in fast paras they use (0,)")
    parse.add_argument('--surrounding',default = (3,4),type=tuple,help = "surrounding params. Itti default params are (3, 4),in fast pami they use (4,)")
...
def main(args:argparse.Namespace):
    ...
    O_bar = normalize_img(O_bar_synthesis(Is,args,gabor_lib,norm_lib),norm_lib)
    ...
    if args.double_height:
        ...
        if args.output == "write":
            ...
            S_bar = interference_function(static_saliency_map, (height//2,width//2)) # add central gaussian
    ...
    for _count in tqdm.trange(int(frame_number)): #in tqdm.trange(int(frame_number)):
        ...
        O_bar = normalize_img(O_bar_synthesis(Is,args,gabor_lib,norm_lib),norm_lib)
        # static_saliency_map = addition(normalize_img(I_bar,norm_lib) + normalize_img(C_bar,norm_lib) , normalize_img(O_bar,norm_lib))
        static_saliency_map = addition(I_bar/(np.sum(I_bar)+1e-3)+ C_bar/(np.sum(C_bar)+1e-3), (O_bar/np.sum(O_bar)+1e-3))# normalize_img(C_bar,norm_lib)
        # static_saliency_map = np.where(static_saliency_map > np.percentile(static_saliency_map,90),static_saliency_map,0)
        # fig,axs = plt.subplots(2,3)
        # axs[0,0].imshow(static_saliency_map)
        # axs[0,0].set_title("current saliency map")
        # axs[1,0].imshow(addition(I_bar+C_bar,O_bar))
        # axs[1,0].set_title("former saliency map")
        # axs[0,1].imshow(I_bar)
        # axs[0,1].set_title("I bar")
        # axs[1,1].imshow(C_bar)
        # axs[1,1].set_title("C bar")
        # axs[0,2].imshow(O_bar)
        # axs[0,2].set_title("O bar")
        # axs[1,2].imshow(frame)
        # axs[1,2].set_title("Original Frame")
        # plt.show()
        ...
        if args.continuity:
            if _count == 0:
                tmp_static = norm_static / (1-tmp_save_factor)
            else:
                tmp_static = tmp_static * tmp_save_factor + norm_static
                norm_static = tmp_static * (1-tmp_save_factor)
        
        # norm_dynamic *= 0
        # S_bar =  non_linear(norm_static)
        S_bar = norm_static

        
        S_bar = np.uint8((S_bar - np.min(S_bar))/(np.max(S_bar) - np.min(S_bar)+1e-5) * 255)
