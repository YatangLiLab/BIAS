...
from utils import read_video, normalize_img, addition,interference_function, initialize_gabor_lib,load_maximum_dll,multation, eight_pyrimid_built
...
def small_gaussian_pyramid(image,scale=16):
    gaussian_kernel= cv2.getGaussianKernel(int(np.ceil(scale * 2)),scale).ravel()
    gaussian_kernel /= np.max(gaussian_kernel)
    padd_img = np.pad(image,(pad_width:=(scale-1)//2),mode='mean')
    filtered_img = ndimage.convolve1d(ndimage.convolve1d(padd_img,gaussian_kernel,axis=0),gaussian_kernel,axis=1)[pad_width:-pad_width, pad_width:-pad_width]
    rst_img = [0,image,filtered_img]
    return rst_img
def diff_resolution(image,args,ifshow=False):
    '''
    create different resolution for single image
    '''
    ...
    r, g, b = map(small_gaussian_pyramid, [r,g,b])
    Is = [np.uint8(0.299 * r[i] + 0.587 * g[i] + 0.114 * b[i]) for i in range(len(r))] # standard method of calculate Intensity.
...
def parse_args():
    ...
    # 
    parse.add_argument('--video_path', default="E:\\your_path\\video\\video\\video\\003.avi", type=str, help='path of sample video') 
    # parse.add_argument('--video_path', default="E:\\your_path\\CarCrashVideo\\sample_video.mp4", type=str, help='path of sample video') 
    parse.add_argument('--generate_name',default = "result_video\\DoG_both003.mp4",type=str,help = 'default generate name')
    ...
    parse.add_argument('--surrounding',default = (1,),type=lambda s:tuple(int(item) for item in s.split(',')[:-1]),help = "surrounding params. Itti default params are (3, 4),in fast pami they use (4,)")
...
def main(args:argparse.Namespace):
    ...
    rIs, Rs, Gs, Bs, Ys, rDs, Is = diff_resolution(resized_image,args) #Itti_down_sampling(resized_image,args) # intensity channels and color channels
    Is = eight_pyrimid_built(Is[1])
    ...
    for _count in tqdm.trange(int(frame_number)): # in range(int(frame_number)): #
        ...
        # S_bar = norm_static# addition(norm_static,norm_dynamic)
        #a_sd = 2.80
        #a_s = 0.69
        #a_d = 0.84
        #a_constant = 0
