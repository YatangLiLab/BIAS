...
def fast_exp(x, lut, lut_min, lut_step):
    """
    exp(x), x \in [lut_min, 0]
    lut      : precomputed (float32)
    lut_min  : min_val (一般 -12)
...
if __name__ == '__main__':
    ...
    axs[1].scatter(traj[0, 1], traj[0, 0], c='cyan', label='Start point')
    axs[1].scatter(traj[-1, 1], traj[-1, 0], c='pink', label='End point')
